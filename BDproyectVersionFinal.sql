...
INSERT INTO MENSUALIDAD (monto, observacion, estado, fechaPago, totalPago)
VALUES (500.00, 'Pago puntual', 'Pagado', '2024-01-05', 500.00),
       (450.00, 'Descuento aplicado', 'Pagado', '2024-02-05', 450.00),
       (500.00, 'Pago puntual', 'Pagado', '2024-03-05', 500.00),
       (480.00, 'Descuento aplicado', 'Pagado', '2024-04-05', 480.00),
       (500.00, 'Pago puntual', 'Pagado', '2024-05-05', 500.00),
       (470.00, 'Descuento aplicado', 'Pagado', '2024-06-05', 470.00),
       (500.00, 'Pago puntual', 'Pagado', '2024-07-05', 500.00),
       (460.00, 'Descuento aplicado', 'Pagado', '2024-08-05', 460.00),
       (500.00, 'Pago puntual', 'Pagado', '2024-09-05', 500.00),
       (490.00, 'Descuento aplicado', 'Pagado', '2024-10-05', 490.00),
       (500.00, 'Pago puntual', 'Pagado', '2024-11-05', 500.00),
       (450.00, 'Descuento aplicado', 'Pagado', '2024-12-05', 450.00),
       (500.00, 'Pago puntual', 'Pagado', '2025-01-05', 500.00),
       (470.00, 'Descuento aplicado', 'Pagado', '2025-02-05', 470.00),
       (500.00, 'Pago puntual', 'Pagado', '2025-03-05', 500.00),
       (480.00, 'Descuento aplicado', 'Pagado', '2025-04-05', 480.00),
       (500.00, 'Pago puntual', 'Pagado', '2025-05-05', 500.00),
       (460.00, 'Descuento aplicado', 'Pagado', '2025-06-05', 460.00),
       (500.00, 'Pago puntual', 'Pagado', '2025-07-05', 500.00),
       (490.00, 'Descuento aplicado', 'Pagado', '2025-08-05', 490.00);

INSERT INTO USUARIO (nombreUsuario, contrasenia)
VALUES ('jhunior', 'xY!9k#p@'),
       ('cristina', '$3&Lq7@!'),
       ('miguel', 'H@6*sn2p'),
       ('director', 'T#8j@Qf*');

-- Crear Vistas
CREATE VIEW VistaEmpleadosSalario AS
...
FROM EMPLEADO e
         JOIN
     SALARIO s ON e.codEmpleado = s.codEmpleado;

CREATE VIEW VistaCursosMaterias AS
SELECT c.codCurso,
       c.nombreCurso,
       c.nivel,
       m.nombreMateria
FROM CURSOMATERIA cm
         JOIN
     CURSO c ON cm.codCurso = c.codCurso
         JOIN
     MATERIA m ON cm.codMateria = m.codMateria;
...
CREATE FUNCTION SalarioTotalEmpleado(cod INT) RETURNS DECIMAL(10, 2)
BEGIN
    DECLARE total DECIMAL(10, 2);
    SELECT SUM(monto) INTO total FROM SALARIO WHERE codEmpleado = cod;
    RETURN total;
END //
...
CREATE PROCEDURE AsignarEmpleadoCurso(
    IN p_codEmpleado INT,
    IN p_codCursoMateria INT,
    IN p_fechaAsignacion DATE
)
BEGIN
    INSERT INTO ASIGNACIONCURSO (codEmpleado, codCursoMateria, fechaAsignacion)
    VALUES (p_codEmpleado, p_codCursoMateria, p_fechaAsignacion);
END //
...
BEGIN
    DELETE FROM ASIGNACIONCURSO WHERE codEmpleado = p_codEmpleado;
    DELETE FROM SALARIO WHERE codEmpleado = p_codEmpleado;
